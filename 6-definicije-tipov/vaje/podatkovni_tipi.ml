...
let dollar_to_euro (Dollar x) = Euro (x *. 0.8)
let euro_to_dollar (Euro x) = Dollar (x *. 1.2)
...
type magic_counter = {ogenj : int ; led : int ; arkana : int}

let update counter = function
  | Ogenj -> {counter with  ogenj = counter.ogenj + 1}
  | Led -> {counter with led = counter.led + 1}
  | Arkana -> {counter with  arkana = counter.arkana + 1}
