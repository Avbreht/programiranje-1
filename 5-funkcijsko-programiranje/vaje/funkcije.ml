...
let rec mapi f sez =
  let rec mapi' acc = function
  | [] -> []
  | x :: xs -> (f x acc) :: mapi'(acc + 1) xs
  in
  mapi' 0 sez
...
let rec zip list1 list2 =
  ...
  | [], [] -> []
  | _ , [] | [] , _ -> failwith "Different lenghts"
...
let rec apply_sequence f el n =
  let rec apply_aux acc f el n =
    if n < 0 then 
      reverse acc
    else
      apply_aux (el :: acc) f (f el) (n - 1)
  in
  apply_aux [] f el n
